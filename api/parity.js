...
router.get('/eth_getBalance/:address/:tag?', function (req, res, next) {
  getJayson().request('eth_getBalance', [req.params.address, trlBlockParam(req.params.tag)], function (error, result) {
...
router.get('/eth_blockNumber', function (req, res, next) {
  getJayson().request('eth_blockNumber', [], function (error, result) {
    if (result && !result.error) {
...
router.get('/eth_getBlockByNumber/:tag/:bool?', function (req, res, next) {
  getJayson().request('eth_getBlockByNumber', [trlBlockParam(req.params.tag), (req.params.bool && req.params.bool == 'true' ? true : false)], function (error, result) {
    res.json(returnRet(error, result));
  });
});
...
router.get('/eth_getBlockByHash/:tag/:bool?', function (req, res, next) {
  getJayson().request('eth_getBlockByHash', [req.params.tag, (req.params.bool && req.params.bool == 'true' ? true : false)], function (error, result) {
    res.json(returnRet(error, result));
  });
});
...
router.get('/eth_getUncleByBlockNumberAndIndex/:tag/:index', function (req, res, next) {
  getJayson().request('eth_getUncleByBlockNumberAndIndex', [trlBlockParam(req.params.tag), req.params.index], function (error, result) {
    res.json(returnRet(error, result));
  });
});
...
router.get('/eth_getBlockTransactionCountByNumber/:tag', function (req, res, next) {
  getJayson().request('eth_getBlockTransactionCountByNumber', [trlBlockParam(req.params.tag)], function (error, result) {
    if (result && !result.error) {
      result.result = (new BigNumber(result.result)).toNumber();
    }
    res.json(returnRet(error, result));
...
router.get('/eth_getTransactionByHash/:txhash', function (req, res, next) {
  getJayson().request('eth_getTransactionByHash', [req.params.txhash], function (error, result) {
    res.json(returnRet(error, result));
  });
});
...
router.get('/eth_getTransactionByBlockNumberAndIndex/:tag/:index', function (req, res, next) {
  getJayson().request('eth_getTransactionByBlockNumberAndIndex', [trlBlockParam(req.params.tag), req.params.index], function (error, result) {
    res.json(returnRet(error, result));
  });
});
...
router.get('/eth_getTransactionCount/:address/:tag?', function (req, res, next) {
  getJayson().request('eth_getTransactionCount', [req.params.address, trlBlockParam(req.params.tag)], function (error, result) {
    res.json(returnRet(error, result));
  });
});
...
router.get('/eth_sendRawTransaction/:hex', function (req, res, next) {
  getJayson().request('eth_sendRawTransaction', [req.params.hex], function (error, result) {
    res.json(returnRet(error, result));
  });
});
...
router.get('/eth_getTransactionReceipt/:txhash', function (req, res, next) {
  getJayson().request('eth_getTransactionReceipt', [req.params.txhash], function (error, result) {
    res.json(returnRet(error, result));
  });
});
...
router.get('/eth_getCode/:address/:tag?', function (req, res, next) {
  getJayson().request('eth_getCode', [req.params.address, trlBlockParam(req.params.tag)], function (error, result) {
    res.json(returnRet(error, result));
  });
});
...
router.get('/eth_getStorageAt/:address/:position/:tag?', function (req, res, next) {
  getJayson().request('eth_getStorageAt', [req.params.address, '0x'.concat(parseInt(req.params.position, 10).toString(16)), trlBlockParam(req.params.tag)], function (error, result) {
    res.json(returnRet(error, result));
  });
});
...
router.get('/eth_gasPrice', function (req, res, next) {
  getJayson().request('eth_gasPrice', [], function (error, result) {
    if (result && !result.error) {
      result.result = (new BigNumber(result.result)).toNumber();
    }
    res.json(returnRet(error, result));
...
router.get('/eth_estimateGas/:to/:value/:gasPrice/:gas', function (req, res, next) {
  ...
  getJayson().request('eth_estimateGas', [tx, trlBlockParam(req.params.tag)], function (error, result) {
    res.json(returnRet(error, result));
  });
});
