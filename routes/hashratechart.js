  ...
  async.waterfall([
    ...
    function (callback) {
      client.lrange(pre_fix_chart.concat("xData"), 0, -1, function (err, result) {
        function numberingArrayElements(element, index, array) {
          data.xData.push(Number(element));
        }
        result.forEach(numberingArrayElements);
        //console.log("data.xData: " + data.xData.length + " / " + result.length);
        callback(err);
      });
    },
    function (callback) {
      client.lrange(pre_fix_chart.concat("xBlocknumber"), 0, -1, function (err, result) {
        function numberingArrayElements(element, index, array) {
          data.xBlocknumber.push(Number(element));
        }
        result.forEach(numberingArrayElements);
        //console.log("data.xBlocknumber: " + data.xBlocknumber.length + " / " + result.length);
        callback(err);
      });
    },
    function (callback) {
      client.lrange(pre_fix_chart.concat("xNumberOfBlocks"), 0, -1, function (err, result) {
        function numberingArrayElements(element, index, array) {
          data.xNumberOfBlocks.push(Number(element));
        }
        result.forEach(numberingArrayElements);
        callback(err);
      });
    ...
    function (callback) {
      client.lrange(pre_fix_chart.concat("BlockTime"), 0, -1, function (err, result) {
        function numberingArrayElements(element, index, array) {
          data.datasets[0].data.push(Number(element));
        }
        result.forEach(numberingArrayElements);
        //console.log("data.datasets[0].data: " + data.datasets[0].data.length + " / " + result.length);
        callback(err);
      });
    },
    function (callback) {
      client.lrange(pre_fix_chart.concat("Difficulty"), 0, -1, function (err, result) {
        function numberingArrayElements(element, index, array) {
          data.datasets[1].data.push(Number(element));
        }
        result.forEach(numberingArrayElements);
        //console.log("data.datasets[1].data: " + data.datasets[1].data.length + " / " + result.length);
        callback(err);
      });
    },
    function (callback) {
      client.lrange(pre_fix_chart.concat("NetHashrate"), 0, -1, function (err, result) {
        function numberingArrayElements(element, index, array) {
          data.datasets[2].data.push(Number(element));
        }
        result.forEach(numberingArrayElements);
        //console.log("data.datasets[2].data: " + data.datasets[2].data.length + " / " + result.length);
        callback(err);
      });
    },
    function (callback) {
      data.blockCount = data.dbLastBlock - data.dbChartLastBlock;
      //console.log("blockCount: ", data.blockCount, "dbLastBlock: ", data.dbLastBlock, "dbChartLastBlock: ", data.dbChartLastBlock);
      ...
      async.times(data.blockCount, function (n, next) {
        ...
        if (field > 0) {
          ...
          client.hmget(fieldkey, 'timestamp', 'difficulty', 'number', function (err, block_info) {
            if (err || !block_info) {
    ...
    for (var i = 0; i < dbSaveDatas.xData.length; i++) {
